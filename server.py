#!/usr/bin/env python3
"""
CoinMarketCap Pro v6 MCP Server - FastMCP with D402 Transport Wrapper
...
- COINMARKETCAP_PRO_V6_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('coinmarketcap-pro-v6_mcp')
...
API_KEY = os.getenv("COINMARKETCAP_PRO_V6_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  COINMARKETCAP_PRO_V6_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"CoinMarketCap Pro v6 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("CoinMarketCap Pro v6 MCP Server", host="0.0.0.0")
...
# - COINMARKETCAP_PRO_V6_API_KEY: Server's internal API key for payment mode
#
...
async def coinmarketcap_id_map_dc8b(
    context: Context,
    listing_status: str = "active",
    slug: Optional[str] = None,
    start: int = 1,
    ...
    Example Usage:
        await coinmarketcap_id_map_dc8b()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_dc8b: {e}")
...
async def coinmarketcap_id_map_6136(
    context: Context,
    start: int = 1,
    limit: Optional[int] = None,
    sort: str = "id",
    ...
    Example Usage:
        await coinmarketcap_id_map_6136()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_6136: {e}")
...
async def metadata_v1_deprecated(
    ...
    Example Usage:
        await metadata_v1_deprecated(id="1")
...
async def ohlcv_historical_v1_deprecated(
    ...
    Args:
        ...
        time_start: Timestamp (Unix or ISO 8601) to start returning OHLCV time periods for. Only the date portion of the timestamp is used for daily OHLCV so it's recommended to send an ISO date format like "2018-09-19" without time. (optional) Examples: "1"
        time_end: Timestamp (Unix or ISO 8601) to stop returning OHLCV time periods for (inclusive). Optional, if not passed we'll default to the current time. Only the date portion of the timestamp is used for daily OHLCV so it's recommended to send an ISO date format like "2018-09-19" without time. (optional) Examples: "1"
        ...
        convert: By default market quotes are returned in USD. Optionally calculate market quotes in up to 3 fiat currencies or cryptocurrencies. (optional) Examples: "1"
        convert_id: Optionally calculate market quotes by CoinMarketCap ID instead of symbol. This option is identical to `convert` outside of ID format. Ex: convert_id=1,2781 would replace convert=BTC,USD in your query. This parameter cannot be used when `convert` is used. (optional) Examples: "1"
    ...
    Example Usage:
        await ohlcv_historical_v1_deprecated(id="1")
...
async def ohlcv_latest_v1_deprecated(
    ...
    Args:
        ...
        convert: Optionally calculate market quotes in up to 120 currencies at once by passing a comma-separated list of cryptocurrency or fiat currency symbols. Each additional convert option beyond the first requires an additional call credit. A list of supported fiat options can be found [here](#section/Standards-and-Conventions). Each conversion is returned in its own "quote" object. (optional) Examples: "1"
        convert_id: Optionally calculate market quotes by CoinMarketCap ID instead of symbol. This option is identical to `convert` outside of ID format. Ex: convert_id=1,2781 would replace convert=BTC,USD in your query. This parameter cannot be used when `convert` is used. (optional) Examples: "1"
        skip_invalid: Pass `true` to relax request validation rules. When requesting records on multiple cryptocurrencies an error is returned if any invalid cryptocurrencies are requested or a cryptocurrency does not have matching records in the requested timeframe. If set to true, invalid lookups will be skipped allowing valid cryptocurrencies to still be returned. (optional, default: True)
    ...
    Example Usage:
        await ohlcv_latest_v1_deprecated(id="1")
...
async def price_performance_stats_v1_deprecated(
    ...
    Args:
        ...
        convert_id: Optionally calculate quotes by CoinMarketCap ID instead of symbol. This option is identical to `convert` outside of ID format. Ex: convert_id=1,2781 would replace convert=BTC,USD in your query. This parameter cannot be used when `convert` is used. (optional) Examples: "1"
    ...
    Example Usage:
        await price_performance_stats_v1_deprecated(id="1")
...
async def quotes_historical_v1_deprecated(
    ...
    Args:
        ...
        time_start: Timestamp (Unix or ISO 8601) to start returning quotes for. Optional, if not passed, we'll return quotes calculated in reverse from "time_end". (optional) Examples: "1"
        time_end: Timestamp (Unix or ISO 8601) to stop returning quotes for (inclusive). Optional, if not passed, we'll default to the current time. If no "time_start" is passed, we return quotes in reverse order starting from this time. (optional) Examples: "1"
        ...
        convert: By default market quotes are returned in USD. Optionally calculate market quotes in up to 3 other fiat currencies or cryptocurrencies. (optional) Examples: "1"
        convert_id: Optionally calculate market quotes by CoinMarketCap ID instead of symbol. This option is identical to `convert` outside of ID format. Ex: convert_id=1,2781 would replace convert=BTC,USD in your query. This parameter cannot be used when `convert` is used. (optional) Examples: "1"
    ...
    Example Usage:
        await quotes_historical_v1_deprecated(id="1")
...
async def quotes_latest_v1_deprecated(
    ...
    Example Usage:
        await quotes_latest_v1_deprecated(id="1")
...
async def quotes_historical_cw9b(
    context: Context,
    time_start: Optional[str] = None,
    time_end: Optional[str] = None,
    count: float = 10,
    ...
    try:
        ...
        logger.error(f"Error in quotes_historical_cw9b: {e}")
...
async def quotes_latest_3b5d(
    ...
    try:
        ...
        logger.error(f"Error in quotes_latest_3b5d: {e}")
...
async def ohlcv_historical_v2(
    ...
    Example Usage:
        await ohlcv_historical_v2(id="1")
...
async def price_performance_stats_v2(
    ...
    Example Usage:
        await price_performance_stats_v2(id="1")
...
async def quotes_historical_v2(
    ...
    Example Usage:
        await quotes_historical_v2(id="1")
...
async def quotes_historical_v3(
    ...
    Example Usage:
        await quotes_historical_v3(id="1")
...
async def fcas_quotes_latest_deprecated(
    ...
    Example Usage:
        await fcas_quotes_latest_deprecated(id="1")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="coinmarketcap-pro-v6-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "coinmarketcap-pro-v6-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting CoinMarketCap Pro v6 MCP Server")
    logger.info("="*80)
